...
int scene_intersection_tri( ray_t *ray, __global const vec3 *verts, __global const uint *tri_indices, const uint n_geometry)
{
	float u, v;
	triangle_t tri;
	int hitObject = -1;
 ...
 */
bool visibility_test_tri(const ray_t *ray, __global const vec3 *verts, __global const uint *tri_indices, const uint n_geometry)
{
	float u, v;
	triangle_t tri;
	for (int geom_idx = 0; geom_idx < n_geometry; ++geom_idx) {
 ...
 */
float4 sample_direct_illumination_tri(const hit_info_t *hit, __global const vec3 *verts, __global const uint *tri_indices, const uint n_geometry, __constant Sphere *lights, const uint n_lights, seed_value_t *seed) {
	ray_t ray;
