...
inline GlutWindow *GlutWindow::getGlutWindow() 
{
  ...
  if (gWindowID < MAX_GLUT_WINDOWS && gWindowID >= 0 && glutWindows[gWindowID] != NULL) 
    {
...
GlutWindow::GlutWindow(unsigned int const width, unsigned int const height, char const * title)
{
  this->width = width;
  this->height = height;
...
void GlutWindow::glutDisplayCallback()
{
  std::cerr << "Display Not Defined!!!!!" << std::endl;
}
