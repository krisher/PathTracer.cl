...
GlutCLWindow::GlutCLWindow(int width, int height) : GlutWindow(width,height, "OpenCL Ray Tracer")
{
  /*
   * GL Context must be current for GL sharing to work...
   */
  //  rayTracer = RayTracerCL();
  rayTracer.setCameraSpherical(gmtl::Point3f(0,-4,-1), 75.0f, 180.0f, 5);
...
void GlutCLWindow::allocatePBO() 
{
  ...
  std::cout << "Allocating PBO with " << width * height * sizeof(GLfloat) * 4 << " bytes" << std::endl;
  ...
  if (rayTracer.supportsGLSharing())
    {
      int err = 0;
      clPBO = clCreateFromGLBuffer((rayTracer.getCLContext())(), CL_MEM_READ_WRITE, pbo, &err );
      std::cout << "Created shared CL/GL buffer." << std::endl;
      if (err)
	{
	  std::cerr << "Error binding OpenGL PBO to OpenCL Buffer (Error "<< err <<"), falling back to failsafe method." << std::endl;
	  std::cerr << "PBO ID: " << pbo << std::endl;
	  std::cerr << "CL Ctx: " << (rayTracer.getCLContext())()  << std::endl;
	}
    }
  else 
    {
      try 
	{
	  clPBOBuff = new cl::Buffer((rayTracer.getCLContext()), CL_MEM_WRITE_ONLY, width * height * sizeof(GLfloat) * 4);
	  clPBO = (*clPBOBuff)();
	} 
      catch (cl::Error err) 
	{
	  std::cerr << "Error allocating CL PBO: " << err.what() << " (" << err.err() << ")" << std::endl;
	  throw (err);
	}
    }
  /*
  ...
  if (reallocPBO) 
    {
      ...
      rayTrace();
      reallocPBO = false;
      if (maxProgression > 0) glutPostRedisplay();
    } 
  ...
  if (rayTracer.supportsGLSharing()) 
    {
      ...
      clEnqueueAcquireGLObjects(rayTracer.getCLCommandQueue()(), 1, &clPBO, 0, 0, 0);
    } 
  ...
  if (rayTracer.supportsGLSharing()) 
    {
       ...
       */
      clEnqueueReleaseGLObjects(rayTracer.getCLCommandQueue()(), 1, &clPBO, 0, 0, 0);
    }
  else 
    {
      /*
      ...
      GLfloat* ptr = (GLfloat*)glMapBuffer(GL_PIXEL_UNPACK_BUFFER,
					   GL_WRITE_ONLY);
      // Blocking read... 
      rayTracer.getCLCommandQueue().enqueueReadBuffer(*clPBOBuff, CL_TRUE, 0,  width * height * sizeof(GLfloat) * 4, ptr);
